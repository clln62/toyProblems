...
function validSolution(board){
  let answer = true;
  let pointer = {};
  if (board[0][0] === board[1][0] || board[0][0] === board[1][1] || board[0][0] === board[1][2]) {
  return false;
  }
  board.map (function (element) {
    let arr = [];
    function helper (array, pointer) {
      for (let i = 0; i < array.length; i++) {
        if (!arr.includes(array[i]) && i !== pointer[array[i]]) {
          arr.push(array[i])
          pointer[array[i]] = i;
        } else {
          answer = false;
